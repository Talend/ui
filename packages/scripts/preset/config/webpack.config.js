...
function getBabelRc(userLocation) {
	if (fs.existsSync(userLocation)) {
		return JSON.parse(fs.readFileSync(userLocation, 'utf8'));
	}
	return require('./.babelrc.json');
}

const overrideBabelRcLocation = `${process.cwd()}/.babelrc`;
const babelrc = getBabelRc(overrideBabelRcLocation);
