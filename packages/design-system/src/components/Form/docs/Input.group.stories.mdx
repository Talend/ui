...
import { AffixButton } from '../Field/Input/Affix';
...
<Canvas>
	<Story name="button">
		<Form.FieldGroup label="Input" suffix={<AffixButton onClick={() => {}}>Button</AffixButton>}>
			<Form.Text />
		</Form.FieldGroup>
	</Story>
</Canvas>
...
<Canvas>
	<Story name="button--icon">
		<Form.FieldGroup
			...
			suffix={
				<AffixButton onClick={() => {}} icon={'talend-wrench'} hideText>
			...
			return (
				<Form onSubmit={handleSubmit(setFormData)}>
					...
					<Form.FieldGroup
						label="API Key"
						suffix={
							<AffixButton onClick={() => {}} icon="talend-files-o" hideText>
								Copy
							</AffixButton>
						}
		...
		{({ prefix, field, suffix, ...rest }) => {
			const getComponent = name => {
				switch (name) {
					...
					case 'Button':
						return <AffixButton onClick={() => {}}>Next week</AffixButton>;
					default:
						return name;
				}
			};
