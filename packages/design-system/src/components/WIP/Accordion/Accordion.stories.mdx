...
<Use>
	<Use.Do>
		<ul>
			<li>
				The prop <b>a11yId</b> is mandatory ! It is used to populate several aria attributes in the
				panel sub-elements. It must be unique to the panel.
			</li>
		</ul>
	</Use.Do>
</Use>
...
A disabled panel will hide it's chevron and action, not be clickable and will display it's title in light gray.
...
Use the prop <b>size</b> to change the height, font size and icon size of the panel header.
...
The prop <b>action</b> takes a object with 3 atributes :
...
The prop <b>metadata</b> takes an array of ReactElement or String, and display them between the panel title and action.
...
- Allways use the `a11yId` prop to give a unique id to your panels, it will be use to populate several aria attributes
