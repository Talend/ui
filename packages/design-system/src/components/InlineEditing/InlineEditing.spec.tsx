...
context('<InlineEditing />', () => {
	it('should go to edit mode when clicking on the button', () => {
		...
		cy.findByTest('inlineediting.button.edit').click();
		cy.findByTest('inlineediting.input').should('exist').should('have.attr', 'type', 'text');
	});

	it('should go to edit mode when double clicking on the span', () => {
		cy.mount(<InlineEditing label="Edit the value" defaultValue="Lorem Ipsum" />);
		cy.findByTest('inlineediting').dblclick();
		cy.findByTest('inlineediting.input').should('exist').should('have.attr', 'type', 'text');
	});
	...
	it('should validate on Enter', () => {
		...
		cy.findByTest('inlineediting.input')
			...
			.should('have.value', 'blah')
			.type('{enter}');
		cy.findByTest('inlineediting').should('have.text', 'blah');
	});
	...
	it('should not validate on Enter when multiline', () => {
		...
		cy.findByTest('inlineediting.button.edit').click();
		cy.findByTest('inlineediting.textarea')
			.focus()
			.type('{selectall}{del}blah')
			.should('have.value', 'blah')
			.type('{enter}');
	...
	it('should trigger cancel on Enter with focus on cancel button', () => {
		...
		cy.findByTest('inlineediting.textarea')
			...
			.type('{selectall}{del}blah')
			.should('have.value', 'blah');
		cy.findByTest('inlineediting.button.cancel').focus().type('{enter}');
		cy.findByTest('inlineediting').should('have.text', defaultValue);
	});
});
