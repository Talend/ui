		...
		remote(
			...
			(err, cachePath) => {
				defaultDotFiles.forEach(dotfile => {
					this.fs.copy(
						path.join(cachePath, jsFormatterPath, dotfile),
						this.destinationPath(dotfile),
					);
				});
				this.fs.copy(
					path.join(cachePath, jsFormatterPath, prettier),
					this.destinationPath('.prettierrc'),
				);
				this.fs.copy(path.join(cachePath, githubRoot, gitignore), this.destinationPath(gitignore));

				if (this.props.eslint) {
					this.fs.copy(path.join(cachePath, jsFormatterPath, eslint), this.destinationPath(eslint));
				}
				if (this.props.travis) {
					this.fs.copy(path.join(cachePath, jsFormatterPath, travis), this.destinationPath(travis));
				}
				if (this.props.sasslint) {
					this.fs.copy(
						path.join(cachePath, scssFormatterPath, sasslint),
						this.destinationPath(sasslint),
					);
				}
				if (this.props.babelrc) {
					this.fs.copy(
						path.join(cachePath, jsFormatterPath, babelrc),
