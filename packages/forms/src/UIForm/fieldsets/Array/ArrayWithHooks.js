...
export default function ArrayWidget(props) {
	...
	const { displayMode, uiForm, onChange, onFinish } = useContext(UIFormContext);

	const canReorder = schema.reorder !== false;
	const templateId = displayMode ? `array_${displayMode}` : 'array';
	const ArrayTemplate = uiForm.templates[templateId];

	const itemWidget = uiForm.widgets[schema.itemWidget];
