...
class CollapsiblePanelsList extends React.Component {
	...
	toggleRow = (index, measure) => {
		this.setState(state => ({ [index]: !state[index], measured: true }), () => setTimeout(measure, 0));
	};
	render() {
		return (
			<div className={css['collapsible-panels-list']}>
				<VirtualizedList rowHeight={cache.rowHeight} collection={this.props.items} disableHeader>
					<VirtualizedList.Content cellRenderer={props => (
						<CellMeasurer
							cache={cache}
							columnIndex={0}
							key={props.rowIndex}
						...
						>
							{({ measure }) => (
								<div className={css['collapsible-panels-list-row']} style={{ maxHeight: this.state.measured ? 'none' : `${defaultHeight}px` }}>
									<CollapsiblePanel
										{...props.rowData}
