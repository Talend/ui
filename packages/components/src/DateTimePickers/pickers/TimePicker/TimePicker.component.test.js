...
describe('TimePicker component', () => {
	...
	describe('event handlers', () => {
		it('should call onSubmit when select time', () => {
			...
			const instance = mount(<TimePicker onSubmit={onSubmit} />);
			// when
			instance.find('button').at(3).simulate('click');
			// then
			expect(onSubmit).toBeCalledWith(event, { time: { hours: '03', minutes: '00', seconds: '00' } });
		});
		it('should hightlight item matches user input', () => {
			// when
			const instance = mount(<TimePicker onSubmit={jest.fn()} textInput="12:00" />);
			// then
			expect(instance.state().hightlightedItemIndex).toBe(12);
		});
	});
});
