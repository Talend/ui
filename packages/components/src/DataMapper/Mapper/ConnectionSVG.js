...
function renderBezierCurve(params, style) {
	return (
		<g>
			<circle
				className={
					getClassname(Constants.Connection.PART.START, style)
				}
				cx={params.x1}
				cy={params.y1}
				r={getRadius(style)}
			...
			<path
				className={
					getClassname(Constants.Connection.PART.CURVE, style)
				}
				d={params.path}
			/>
			<path
				className={
					getClassname(Constants.Connection.PART.END, style)
				}
				d={params.arrow}
			/>
		</g>
...
function renderConnection(params, style) {
	switch (params.kind) {
		...
		default:
			return (
				<div>Cannot render connection</div>
			);
	}
}
