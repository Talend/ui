...
function storeMapping(dataAccessor, mapping) {
  return mapping.map(item =>
    ({
      sourceId: dataAccessor.getElementId(dataAccessor.getMappedElement(item, Constants.MappingSide.INPUT)),
      targetId: dataAccessor.getElementId(dataAccessor.getMappedElement(item, Constants.MappingSide.OUTPUT)),
    })
  );
}
...
export default class DataAccessorWithUndoRedo extends DataAccessorWrapper {
  ...
  addMapping(mapping, source, target) {
    ...
    const cmd = {
      ...
      label: `Map ${sourceName} to ${targetName}`,
  ...
  removeMapping(mapping, source, target) {
    ...
    const cmd = {
      ...
      label: `Clear mapping between ${sourceName} and ${targetName}`,
      sourceId: this.dataAccessor.getElementId(source),
      targetId: this.dataAccessor.getElementId(target),
    };
    this.undoStack.push(cmd);
