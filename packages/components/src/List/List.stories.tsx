...
function MyCustomRow(props: any) {
	return (
...
function ListColumnChooser({ list, ...rest }: any) {
	const [columnsChooser, setColumnsChooser] = useState(list.columns);
	const onSubmit = (_: any, newColumnsChooser: any) => {
...
function CellWithHello({ cellData }: { cellData: string }) {
...
const overlayAction = {
	...
	overlayPlacement: 'bottom' as const,
...
export const HiddenHeaderLabels: Story = {
	render: () => {
		const customProps = cloneDeep(props);
