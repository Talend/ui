...
describe('ColumnChooser', () => {
	...
	it('should filter the columns by name if an initial filter value is provided', () => {
		...
		// Then
		expect(
			wrapper.find('input[aria-describedby="my-id-body-hide the column Author"]'),
		).toHaveLength(0);
		expect(wrapper.find('input[aria-describedby="my-id-body-hide the column Name"]')).toHaveLength(
			1,
		);
	});
