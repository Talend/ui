 ...
 */
function buildRegExpJsonpath(jsonpath: string) {
...
export const DataTreeWithHighlight: Story = {
	render: () => {
		...
		const onSelect = (_: any, jsonpath: string) => setJsonPathSelection(jsonpath);
		const isUnion = (item: any) => Array.isArray(item.type);
...
export const DataTreeWithoutSemanticAwareness: Story = {
	render: () => {
		const [jsonPathSelection, setJsonPathSelection] = useState("$['category']");
		const layoutCn = classNames(theme['tc-twoviewers-layout'], 'tc-twoviewers-layout');
...
export const DataTreeWithTypeDisplayOnRecords: Story = {
	render: () => (
		<div style={{ height: '100%' }}>
			<RecordsViewer
				...
				sample={hierarchicSample}
				displayTypes
				typesRenderer={(schema: any) => <>- of type {schema.type[0].type}</>}
			/>
		</div>
	),
};
