...
const ColorDescription = ({ token, children, ...rest }: ColorDescriptionProps) => token ? (
    <dl className={S.tokens} {...rest}>
        <div className={S.token}>
            ...
            <dd className={S.tokenValueCss}><code>{('hex' in token) ? (token as ColorToken)?.hex : token?.value}</code></dd>
        </div>
    </dl>
) : (
    <div />
