...
if [ -n "$GH_TOKEN" ]; then
	...
	if [ "$TRAVIS_BRANCH" != 'master' ]; then
		...
		echo "git status"
		git status
		git -c user.name="travis" -c user.email="travis" commit -m "tests: update screenshots from CI [skip ci]"
		echo "✓ Commit updated screenshots to $TRAVIS_BRANCH"

		git add output/
		git -c user.name="travis" -c user.email="travis" commit -m "tests: update code style outputs from CI [skip ci]"
