...
<IconGallery>
	{['talend-error', 'talend-warning', 'talend-check', 'talend-info-circle'].map(
		(iconName, index) => (
...
<ul>
	<li>
		Errors whichdisplay to tell users something serious or less serious has gone wrong and need
