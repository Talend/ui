...
describe('Size', () => {
	...
	const improperTestSize = new Immutable.Map({ width: 10, height: 10 });
	describe('create', () => {
		...
		it('throw if given an improper width', () => {
			...
			// when
			// expect
			expect(() => Size.create(improperWidth, height)).toThrow(isImproperWidth);
		});
		it('throw if given an improper Height', () => {
			// given
			// when
			// expect
			expect(() => Size.create(width, improperHeight)).toThrow(isImproperHeight);
		});
	});
	describe('isSize', () => {
		it('return true if parameter size is a SizeRecord', () => {
