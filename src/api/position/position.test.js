...
describe('Position', () => {
	...
	const improperX = '10';
	const improperY = '50';
	const improperTestPosition = new Immutable.Map({ x: 10, y: 10 });
	describe('create', () => {
		...
		it('throw if given an improper id', () => {
			...
			// when
			// expect
			expect(() => Position.create(improperX, y)).toThrow(isImproperXCoordinate);
		});
		it('throw if given an improper Position', () => {
			// given
			// when
			// expect
			expect(() => Position.create(x, improperY)).toThrow(isImproperYCoordinate);
		});
	});
	describe('isPosition', () => {
		it('return true if parameter position is a PositionRecord', () => {
